...
class BaseModelView extends React.Component {
  ...
  componentWillReceiveProps(newProps) {
    if (
        newProps.source.zip !== this.props.source.zip ||
  ...
  fetch(uri) {
    return RNFS.mkdir(DOCUMENTS_FOLDER).then(() => {
      ...
      return RNFS.exists(targetPath).then((exists) => {
        if (false) {
          //remove fetching cached file as it is leadeing to mixup of files with same name
          return targetPath;
        } else {
          return RNFetchBlob.config({
            fileCache: false, //don't cache files
            path: targetPath,
          })
              .fetch("GET", uri, { cache: "no-cache" })
              .then((res) => {
                return res.path();
              });
        }
